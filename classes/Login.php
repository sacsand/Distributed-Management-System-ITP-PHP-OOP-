...
class Login {
	...
	public static $_login_page_front = "/?page=index";
	public static $_dashboard_front = "/?page=turn";
	public static $_login_front = "userId";
	public static $_login_name  ="userName";
	...
	public static function loginFront($id,$name,$url = null) {
		if (!empty($id)) {
			$url = !empty($url) ? $url : self::$_dashboard_front;
			$_SESSION[self::$_login_front] = $id;
			$_SESSION[self::$_login_name] = $name;
			$_SESSION[self::$_valid_login] = 1;
			session_start();
    		Helper::redirect($url);
                   }
